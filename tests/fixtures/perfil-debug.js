...
async function perfilFotoEscolhida(input) {
  ...
  console.log("DBG1 before try"); toast('Enviando foto…', 'info')
  try {
    console.log("DBG2 reducing"); const blob = await _perfilReduzir(arq); console.log("DBG3 reduced", blob && blob.size)
    const uid  = appState.usuario.id
    const ext  = blob.type === 'image/jpeg' ? 'jpg' : (arq.name.split('.').pop() || 'jpg').toLowerCase()
    const path = `${uid}/${Date.now()}.${ext}`

    console.log("DBG4 uploading", path); const { error: upErr } = await db.storage.from('avatares')
    ...
    console.log("DBG_CATCH", e && e.message, e && e.stack);
