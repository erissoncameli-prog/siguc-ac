-- ─────────────────────────────────────────────────────────────────
-- 039 — pesquisa-status.html: histórico com autorizador,
--       documentos com storage_path, policy anon p/ leitura de docs
-- ─────────────────────────────────────────────────────────────────

-- 1. Atualiza RPC pública: inclui usuario_nome no historico,
--    storage_path/mime/tamanho nos documentos,
--    e nome do assinador da AAP
...
-- 2. Permite que usuários anon gerem signed URLs para documentos
--    de pesquisa (paths têm UUID como prefixo — não adivinháveis)
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'storage'
      ...
      AND policyname = 'pesq_docs_read_anon'
  ) THEN
    CREATE POLICY "pesq_docs_read_anon"
    ON storage.objects FOR SELECT TO anon
    USING (bucket_id = 'pesquisa-documentos');
  END IF;
END $$;
