-- 338_bio_dados_aba_ninhos_validados.sql
-- Dois KPIs novos na aba Dados (sub-aba Ninhos): ninhos VALIDADOS e
-- NÃO VALIDADOS. Fonte: ninhos_quelonios.status_validacao
-- (enum status_validacao_bio: pendente|validado|rejeitado|em_correcao).
--   validados     = status_validacao = 'validado'
--   nao_validados = tudo o mais (pendente + rejeitado + em_correcao + NULL)
-- Respeitam os mesmos filtros já existentes (temporada + espécie).
--
-- Assinatura INALTERADA (uuid, especie_quelonio) -> CREATE OR REPLACE direto,
-- sem DROP (não é overload novo). O ACL (anon revogado, authenticated
-- concedido pela migration 337) persiste no REPLACE; reafirmado ao final
-- por garantia.
...
BEGIN
  ...
  agg AS (
    SELECT
      ...
      COUNT(*) FILTER (WHERE status_validacao = 'validado')               AS validados,
      COUNT(*) FILTER (WHERE status_validacao IS DISTINCT FROM 'validado') AS nao_validados,
  ...
  SELECT jsonb_build_object(
    ...
    'ninhos_validados',         a.validados,
    'ninhos_nao_validados',     a.nao_validados,
