-- ═══════════════════════════════════════════════════════════════
-- SIGUC-AC · Biomonitor — "Ovos viáveis" no mapa (ninho e praia)
-- ───────────────────────────────────────────────────────────────
-- qtd_ovos = postura (histórico, intacto). Agora as RPCs do mapa
-- também expõem:
--   ovos_predados = soma dos descartes por predação
--   ovos_viaveis  = postura − todos os descartes (predação/natural/humana)
-- Assim o painel mostra os ovos caindo após a predação, no ninho e na
-- praia. Depende de 102_bio_mapa_rpcs e 105_visita_predacao_descarte.
-- ═══════════════════════════════════════════════════════════════
...
BEGIN
  SELECT jsonb_agg(
    jsonb_build_object(
      ...
      -- Perdas por predação e ovos viáveis (postura − descartes)
