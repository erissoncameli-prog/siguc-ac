-- ═══════════════════════════════════════════════════════════════
-- SIGUC-AC · Biomonitor — bio_analise_detalhada ignorava a
-- biometria individual (biometrias_individuais/filhotes_bercario)
-- ───────────────────────────────────────────────────────────────
-- A seção "Crescimento em berçário" do Relatório Científico (e do
-- Relatório Comparativo) usa bio_analise_detalhada(), cujo bio_serie
-- só lia ocorrencias_bercario (tipo='biometria', amostragem por lote).
-- bio_relatorio_completo() já foi corrigida (149/150) para juntar as
-- duas fontes em crescimento_por_especie/biometria_serie/ganho_por_
-- bercario, mas essa função ficou para trás — resultado: temporadas
-- com biometria só individual (ex.: 2025/2026) apareciam como "sem
-- dados de crescimento" mesmo com milhares de medições registradas.
--
-- Fix: acrescenta ao bio_serie a mesma UNION ALL com
-- biometrias_individuais/filhotes_bercario já usada em
-- bio_relatorio_completo. bio_taxa e tam_soltura (que derivam de
-- bio_serie por MIN/MAX de data por lote) passam a considerar os
-- pontos individuais automaticamente, sem mudança de lógica própria.
-- Aditiva — reproduz fielmente o restante da função vigente.
-- ═══════════════════════════════════════════════════════════════
...
BEGIN
  ...
  bio_serie AS (
    ...
    -- Biometria individual (132) — mesma fonte já usada em bio_relatorio_completo
    SELECT
      l.id AS lote_id, l.bercario_nome, n.especie,
