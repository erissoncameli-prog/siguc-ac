-- ═══════════════════════════════════════════════════════════════════
-- SIGUC-AC · Biomonitor — relatório: uc_id da praia como autoritativa
--            + predação de ovos (unifica 108 + correção de uc_id)
-- ───────────────────────────────────────────────────────────────────
-- Bug: o app de campo gravava ninhos_quelonios.uc_id com a UC de
-- lotação do monitor (BioApp.monitor.uc_id), não a UC da praia onde
-- o ninho foi registrado. Monitores que trabalhavam em praias de
-- outra UC (ou sem UC) acabavam com uc_id errado — e o relatório,
-- que priorizava COALESCE(n.uc_id, p.uc_id), excluía esses ninhos
-- ao filtrar pela UC correta (ex.: ARIE Japiim-Pentecoste retornando
-- zero, pois os ninhos ficavam com uc_id da APA Lago do Amapá).
--
-- Mudanças:
--   1. Corrige dados históricos: ninhos cujo uc_id difere do uc_id
--      da praia vinculada são atualizados.
--   2. bio_relatorio_completo (5 args): inverte a prioridade para
--      COALESCE(p.uc_id, n.uc_id) — a UC da praia é autoritativa.
--      Mantém total_ovos_predados / total_ovos_viaveis da migração 108.
-- (O app de campo já foi corrigido para gravar uc_id a partir da
--  praia selecionada — não da UC de lotação do monitor.)
-- ═══════════════════════════════════════════════════════════════════
...
WHERE p.id = n.praia_id
  AND n.uc_id IS DISTINCT FROM p.uc_id;

-- ── 2. Recria bio_relatorio_completo com prioridade de uc_id correta ─

DROP FUNCTION IF EXISTS public.bio_relatorio_completo(uuid, uuid, uuid, uuid, tipo_localizacao_praia);

CREATE FUNCTION public.bio_relatorio_completo(
...
BEGIN
  ...
  WITH base_ids AS (
    ...
    WHERE
      ...
      -- praia é a fonte autoritativa de uc_id; n.uc_id é fallback
      AND (p_uc_id IS NULL OR COALESCE(p.uc_id, n.uc_id) = p_uc_id)
  ...
  base AS (
    SELECT
      ...
      COALESCE(p.uc_id, n.uc_id) AS uc_id,
    ...
    LEFT JOIN praias_monitoramento p   ON p.id = n.praia_id
    LEFT JOIN unidades_conservacao uc  ON uc.id = COALESCE(p.uc_id, n.uc_id)
  ...
  SELECT jsonb_build_object(
    'kpis', (
      SELECT jsonb_build_object(
        ...
        'total_filhotes_vivos', a.total_filhotes_vivos, 'total_filhotes_mortos', a.total_filhotes_mortos,
        ...
        'bercario_total_soltado', ba.total_soltado, 'bercario_mortalidade', ba.total_mortalidade,
        'taxa_sobrevivencia_bercario_pct', ROUND(100.0 * ba.total_soltado / NULLIF(ba.total_entrada, 0), 1),
        'taxa_mortalidade_bercario_pct', ROUND(100.0 * ba.total_mortalidade / NULLIF(ba.total_entrada, 0), 1),
...
GRANT EXECUTE ON FUNCTION public.bio_relatorio_completo(uuid, uuid, uuid, uuid, tipo_localizacao_praia) TO authenticated;
