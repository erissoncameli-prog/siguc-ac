-- ═══════════════════════════════════════════════════════════
-- SIGUC-AC · Qualidade da Água — a mensagem do alerta de sólidos em
-- suspensão para de acusar erro de digitação. Só o TEXTO muda: o
-- limiar (v_sst / v_turb < 0.01, turbidez > 10), o tipo ('unidade') e
-- o nível ('confirmar') ficam idênticos, e nenhuma coleta muda de
-- status — as 228 em quarentena por sólidos seguem exatamente como
-- estão.
--
-- POR QUÊ. O texto dizia "provável valor em g/L lançado como mg/L",
-- que aponta para a DIGITAÇÃO. As duas coisas são verdade, e o texto
-- só contemplava uma:
--   · a migration 318 confirmou 108 linhas da série exatamente 1000x
--     fora — ali o erro de unidade no lançamento era real;
--   · os laudos do QUILAB conferidos nesta rodada
--     (tests/fixtures/laudos/) imprimem, eles mesmos, "Sólidos
--     Suspensos — mg/l — 0,297" e "0,039" — ali o banco reproduz o
--     laudo fielmente, e mandar conferir a digitação faz o técnico
--     procurar onde não há o que achar. 0,039 mg/L de suspensão com
--     214 mg/L de dissolvidos não é crível, mas isso é pergunta para
--     o LABORATÓRIO.
-- A mensagem nova separa os dois ramos e diz o que fazer em cada um.
--
-- Recriada a partir do corpo aplicado pela migration 330, com essa
-- única alteração. Assinatura idêntica — CREATE OR REPLACE substitui
-- de fato, sem overload (lição da 178/224).
-- ═══════════════════════════════════════════════════════════
...
BEGIN
  ...
  END IF;
  -- Sólidos em suspensão × turbidez. A redação anterior afirmava
  -- "provável valor em g/L lançado como mg/L", ou seja, acusava erro de
  -- LANÇAMENTO. Isso é verdade em parte dos casos (a 318 confirmou 108
  -- linhas exatamente 1000x fora) e falso em outros: os laudos do QUILAB
  -- conferidos imprimem, eles mesmos, "Sólidos Suspensos — mg/l — 0,297"
  -- e "0,039". Mandar conferir a digitação, que está certa, faz o técnico
  -- procurar no lugar errado. A mensagem passa a separar os dois ramos.
  IF v_sst IS NOT NULL AND v_turb IS NOT NULL AND v_turb > 10 AND v_sst > 0
    ...
    v_out := v_out || jsonb_build_object('parametro','solidos_suspensao_totais','tipo','unidade','nivel','confirmar',
      'mensagem', format('Sólidos em suspensão (%s) muito baixos para a turbidez medida (%s UNT). Confira o laudo: se lá o valor estiver em g/L, é erro de unidade no lançamento; se o próprio laudo trouxer este valor em mg/L, a pendência é do laboratório.',
