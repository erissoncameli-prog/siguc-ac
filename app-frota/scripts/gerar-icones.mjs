// ── SIGUC Frota — gerador de ícones (sem dependências) ──────────
// Diferente do Biomonitor (que recorta um emblema circular de uma arte
// fonte em app-biomonitor/resources/), o Frota já tem ícones prontos e
// opacos em pwa/icons/ (gerados para o PWA): icon-frota-512.png (fundo
// claro) e icon-frota-maskable-512.png (fundo verde do tema, já com a
// margem de segurança do padrão maskable). Este script só reamostra
// essas duas artes para os tamanhos do Android — não há detecção de
// círculo nem composição, ambas já são quadrados opacos.
//   • icon-frota-512.png            → mipmap ic_launcher / ic_launcher_round
//   • icon-frota-maskable-512.png   → mipmap ic_launcher_foreground (adaptativo)
...
// Verde do tema Frota (theme_color do manifest-frota.json) — usado no fundo
// do launch screen.
const VERDE = [0x1F, 0x4E, 0x2C]   // #1F4E2C
...
// ── Execução ───────────────────────────────────────────────────
console.log('Gerando ícones do Frota a partir de pwa/icons/icon-frota-*.png…')
const opaco = decodePng(readFileSync(join(PWA, 'icon-frota-512.png')))
const maskable = decodePng(readFileSync(join(PWA, 'icon-frota-maskable-512.png')))
...
for (const [dpi, tam, tamFg] of DPIS) {
  ...
  gravar(join(RES, `mipmap-${dpi}/ic_launcher_round.png`),      encodePng(tam,   tam,   render(opaco, tam, 1)))
  gravar(join(RES, `mipmap-${dpi}/ic_launcher_foreground.png`), encodePng(tamFg, tamFg, render(maskable, tamFg, 1)))
}

// Splash: fundo verde sólido do tema Frota (esticado no launch — sólido não distorce)
...
console.log('Ícones do Frota gerados.')
